...
const Header = ({ onNote }) => {
  ...
  function ChangeColor() {
    const newTheme = background.value === "light" ? "dark" : "light";
  ...
  useEffect(() => {
    ...
    if (savedTheme === "dark" && background.value !== "dark") {
      changeBackground({ value: "dark", label: "Dark" });
      document.body.classList.add("dark-mode");
    }
    else if (savedTheme !== "dark" && background.value !== "light") {
  ...
  const getPageTitle = (path) => {
    ...
    // ✅ custom overrides (your control)
    const customTitles = {
      "Procurement-Accounts": "Procurement & Accounts",
      "Requisition-Pending-By-Boss": "Requisition Approvals",
      "SuperAdminList": "Purchase Orders Approvals",
      "SuperAdmin-Repair-List": "Repair Orders Approvals",
      "MachineDeatilsWithDriver":'MachineDriverOverview',
      "AssetVehicleList":"Machine Allocation",
    };

    // ✅ if custom exists → return it
    if (customTitles[page]) return customTitles[page];
          ...
          <div className="collapse navbar-collapse justify-content-between">
            <div className="header-left">
              <div className="dashboard_bar" style={{ textTransform: "capitalize" }}>
                {getPageTitle(path) || "Dashboard"}
              </div>
            </div>
            <h3 className="text-center me-1 ">Indigo Management</h3>
